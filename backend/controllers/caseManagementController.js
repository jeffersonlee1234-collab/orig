...
exports.getAllCases = async (req, res) => {
  try {
    ...
    const [
      ...
      db.query(`SELECT * FROM solo_parent_child_welfare_applications WHERE (module_type = 'SOLO_PARENT' OR module_type IS NULL) AND LOWER(COALESCE(application_status, '')) IN ('approved', 'completed', 'for_release', 'released', 'active') ORDER BY created_at DESC`).catch(() => ({ rows: [] })),
      db.query(`SELECT * FROM solo_parent_child_welfare_applications WHERE module_type = 'CHILD_WELFARE' AND (LOWER(COALESCE(application_status, '')) IN ('approved', 'completed', 'for_release', 'released', 'active') OR LOWER(COALESCE(category_title, '')) LIKE '%nutrition%' OR LOWER(COALESCE(primary_reason_for_assistance, '')) LIKE '%nutrition%') ORDER BY created_at DESC`).catch(() => ({ rows: [] })),
